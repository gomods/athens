...
2.  为了正确地为 SVN 服务器创建认证文件，您需要向它们进行认证，并让 svn 构建正确的哈希文件。
...
3.  正确认证后，我们希望将 .subversion 目录与 Athens 代理服务器共享，以便重复使用这些凭证。下面我们将其作为卷挂载到代理容器中。
...
* Bazaar 不支持 Athens 提供的 Dockerfile，但这些说明对使用 bazaar 的自定义 Athens 构建有效。*
...
5. 正确设置认证后，我们希望将 bazaar 配置目录与 Athens 代理服务器共享，以便重复使用这些凭证。下面我们将其作为卷挂载到代理容器中。
...
本节最初是为了描述配置 Athens git 客户端通过 SSH 而非 HTTP 从 Atlassian Bitbucket 本地实例获取特定的 Go 导入。经过一些调整，它可能可以为配置 Athens 代理访问托管的 Bitbucket 和其他基于 SSH 保护的 VCS 提供指导。如果您的开发工作流程需要通过 SSH 克隆、推送和拉取 Git 仓库，并且希望 Athens 以相同的方式执行，请继续阅读。
...
其中说明 Go 导入的实际内容位于 `https://git.example.com/scm/golibs/logo.git`。将此 URL 与我们通常用于通过 SSH 克隆此项目的 URL（上述）进行比较，可以看出这种 [Git 全局配置](https://git-scm.com/docs/git-config) http 到 ssh 重写规则：
...
因此，为了通过 SSH 获取 `git.example.com/golibs/logo` 依赖项以填充其存储缓存，Athens 最终会调用 git，在给定该重写规则的情况下，git 需要一个与 Bitbucket 上克隆开发人员或服务账户绑定的公钥匹配的 SSH 私钥。这本质上是 github.com 的 SSH 模型。至少，我们需要向 Athens 提供一个 SSH 私钥和 http 到 ssh 的 git 重写规则，并将其挂载到 Athens 容器内供 root 用户使用：
...
我们还提供了一个 ssh config 来绕过主机 SSH 密钥验证，并展示如何将不同主机绑定到不同的 SSH 密钥：
...
作为无密码 SSH 密钥的替代方案，可以使用 [`ssh-agent`](https://en.wikipedia.org/wiki/Ssh-agent)。`ssh-agent` 设置的 `SSH_AUTH_SOCK` 环境变量将传播到 `go mod download`（如果它包含有效 unix 套接字的路径，在跟随符号链接之后）。

因此，如果使用工作的 ssh 代理运行（以及设置了 `SSH_AUTH_SOCK` 的 shell），在设置好与上一节中提到的相同的 `gitconfig` 后，可以按如下方式在 docker 中运行 athens：
...
除了在 GitHub 上使用机器用户外，还可以创建 GitHub App 并通过它进行认证。

在 **Settings > Developer settings > GitHub Apps** 中创建 GitHub App 并安装。需要从 App 中获取 AppID/ClientID、Installation ID 和私钥。

将 [GitHub App Git Credential Helper](https://github.com/bdellegrazie/git-credential-github-app) 安装到您的 `$PATH` 中。Athens Docker 镜像已预装此工具。
