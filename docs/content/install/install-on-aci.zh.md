...
- `AZURE_ATHENS_CONTAINER_NAME` - 容器的名称。 应为字母和数字，可以包含”-”和”_”字符
...
创建ACI容器后，您将看到一个JSON Blob对象，其中包含该容器的公有IP地址。您还将看到正在运行的容器的完全限定域名（[fully qualified domain name](https://en.wikipedia.org/wiki/Fully_qualified_domain_name)，FQDN）（以 `AZURE_ATHENS_DNS_NAME` 为前缀）。
