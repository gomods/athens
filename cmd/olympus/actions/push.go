...
func GetProcessPushNotificationJob(storage storage.Backend, eLog eventlog.Eventlog, dlTimeout time.Duration) worker.Handler {
	return func(args worker.Args) (err error) {
		...
		return mergeDB(ctx, pn.OriginURL, *diff, eLog, storage, module.Download, dlTimeout)
	}
}
