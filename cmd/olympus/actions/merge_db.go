...
func mergeDB(ctx context.Context, originURL string, diff dbDiff, eLog eventlog.Eventlog, storage storage.Backend, downloader module.Downloader, dTimeout time.Duration) error {
	var errors error
	for _, added := range diff.Added {
		if err := add(ctx, added, originURL, eLog, storage, downloader, dTimeout); err != nil {
...
func add(ctx context.Context, event eventlog.Event, originURL string, eLog eventlog.Eventlog, storage storage.Backend, downloader module.Downloader, dTimeout time.Duration) error {
	...
	data, err := downloader(ctx, dTimeout, originURL, event.Module, event.Version)
	if err != nil {
