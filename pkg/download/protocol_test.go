...
func getDP(t *testing.T) Protocol {
	...
	goPrivate := conf.GoPrivate
	fs := afero.NewOsFs()
	mf, err := module.NewGoGetFetcher(goBin, goProxy, goPrivate, fs)
	if err != nil {
		t.Fatal(err)
	}
	...
	st := stash.New(mf, s)
	return New(&Opts{s, st, module.NewVCSLister(goBin, goProxy, goPrivate, fs), nil})
}
