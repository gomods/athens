...
func Upload(ctx context.Context, module, version string, info, mod, zip io.Reader, uploader Uploader, upTimeout time.Duration) error {
	const op errors.Op = "module.Upload"
	tctx, cancel := context.WithTimeout(ctx, upTimeout)
	defer cancel()
