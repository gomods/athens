...
func (s *storageImpl) Info(ctx context.Context, module, vsn string) ([]byte, error) {
	...
	infoReader, err := s.minioClient.GetObject(ctx, s.bucketName, infoPath, minio.GetObjectOptions{})
	if err != nil {
...
func (s *storageImpl) GoMod(ctx context.Context, module, vsn string) ([]byte, error) {
	...
	modReader, err := s.minioClient.GetObject(ctx, s.bucketName, modPath, minio.GetObjectOptions{})
	if err != nil {
		return nil, errors.E(op, err)
	}
...
func (s *storageImpl) Zip(ctx context.Context, module, vsn string) (storage.SizeReadCloser, error) {
	...
	_, err := s.minioClient.StatObject(ctx, s.bucketName, zipPath, minio.StatObjectOptions{})
	if err != nil {
		return nil, errors.E(op, err, errors.KindNotFound, errors.M(module), errors.V(vsn))
	}

	zipReader, err := s.minioClient.GetObject(ctx, s.bucketName, zipPath, minio.GetObjectOptions{})
	if err != nil {
		return nil, errors.E(op, err)
	}
