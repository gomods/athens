...
func TestWithRedisSentinelLock(t *testing.T) {
	...
	wrapper, err := WithRedisSentinelLock(t.Context(), l, []string{endpoint}, masterName, sentinelPassword, "", "", storage.WithChecker(strg), config.DefaultRedisLockConfig())
	if err != nil {
		t.Fatal(err)
	}
	s := wrapper(ms)
...
func TestWithRedisSentinelLockWithRedisPassword(t *testing.T) {
	...
	ms := &mockRedisStasher{strg: strg}
	l := &testingRedisLogger{t: t}
	wrapper, err := WithRedisSentinelLock(t.Context(), l, []string{endpoint}, masterName, sentinelPassword, "", redisPassword, storage.WithChecker(strg), config.DefaultRedisLockConfig())
	if err != nil {
		t.Fatal(err)
	}
	s := wrapper(ms)
...
func TestWithRedisSentinelLockWithUsernameAndPassword(t *testing.T) {
	...
	ms := &mockRedisStasher{strg: strg}
	l := &testingRedisLogger{t: t}
	wrapper, err := WithRedisSentinelLock(t.Context(), l, []string{endpoint}, masterName, sentinelPassword, redisUsername, redisPassword, storage.WithChecker(strg), config.DefaultRedisLockConfig())
	if err != nil {
		t.Fatal(err)
	}
	s := wrapper(ms)
...
func TestWithRedisSentinelLockWithWrongRedisPassword(t *testing.T) {
	...
	_, err = WithRedisSentinelLock(t.Context(), l, []string{endpoint}, masterName, "wrong-sentinel-password", "", redisPassword, storage.WithChecker(strg), config.DefaultRedisLockConfig())
	if err == nil {
		t.Fatal("Expected Connection Error for wrong sentinel password")
	}

	// Test with wrong redis password
	_, err = WithRedisSentinelLock(t.Context(), l, []string{endpoint}, masterName, sentinelPassword, "", "wrong-redis-password", storage.WithChecker(strg), config.DefaultRedisLockConfig())
	if err == nil {
